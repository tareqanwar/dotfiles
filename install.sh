...
migrate_existing_zshrc_aliases() {
  ...
  log "Found existing ~/.zshrc; backing up and migrating alias lines into ~/.zshrc.local"

  local backup_dir="$HOME/.dotfiles-backups"
  mkdir -p "$backup_dir"
  cp "$current_zshrc" "$backup_dir/zshrc.$(date +%Y%m%d%H%M%S).bak"

  local extracted
  extracted="$(mktemp /tmp/zsh-aliases.XXXXXX)"
  grep -E '^[[:space:]]*alias[[:space:]]+[A-Za-z0-9_.-]+=' "$current_zshrc" > "$extracted" || true

  if [[ -s "$extracted" ]]; then
    {
      [[ -f "$local_zshrc" ]] && cat "$local_zshrc"
      echo ""
      echo "# migrated aliases from previous ~/.zshrc"
      cat "$extracted"
    } | awk '!seen[$0]++' > "$local_zshrc"
  fi
